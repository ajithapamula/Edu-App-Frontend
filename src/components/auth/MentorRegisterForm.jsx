...
// MAIN FORM COMPONENT
// ============================================================
const MentorRegisterForm = ({ embedded = false }) => {
  ...
  useEffect(() => {
    const fetchOrganizations = async () => {
      try {
        const data = await apiRequest('/api/mentor/organizations', { method: 'GET' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Backend endpoint: POST /api/mentor/register
      const response = await apiRequest('/api/mentor/register', {
        method: 'POST',
      ...
      setSuccess(
        response.Mentor_Id
          ? `${response.Message || 'Registration successful!'} (Mentor ID: ${response.Mentor_Id})`
  ...
  const formContent = (
    <>
      ...
      <form onSubmit={handleSubmit} style={st.form}>
        ...
        >
          {loading ? <div style={st.spinner} /> : <span>Register as Mentor</span>}
        </button>
      </form>
      ...
      <div style={{ ...st.mainContainer, opacity: mounted ? 1 : 0, transform: mounted ? 'translateY(0)' : 'translateY(48px)' }}>
        ...
        <div style={st.formCard}>
          <div style={st.formHeader}>
            <h1 style={st.formTitle}>Mentor Registration</h1>
            <p style={st.formSubtitle}>Fill in the details to create your mentor profile</p>
          </div>
          {formContent}
        </div>
      </div>
...
const BrandPanel = () => (
  <div style={st.brandPanel}>
    <div style={st.brandContent}>
      ...
      <h2 style={st.brandTitle}>Guide The <br /><span style={st.brandTitleAccent}>Next Generation.</span></h2>
      <p style={st.brandDescription}>Join iMentora as a mentor and help students achieve their career goals through personalized guidance and mock interviews.</p>
    </div>
  </div>
);
