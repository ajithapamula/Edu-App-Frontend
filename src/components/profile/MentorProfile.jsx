// ═══════════════════════════════════════════════════════════════
// MentorProfile.jsx — Place in: src/components/profile/MentorProfile.jsx
// ═══════════════════════════════════════════════════════════════
...
const MentorProfile = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const r = await apiRequest(`/api/admin/org/mentor/list/${user.id}`, { method: 'GET' });
        if (r && !r.Error) setD(r);
      } catch { }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await apiRequest(`/api/admin/org/mentor/photo/view/${user.id}`, { method: 'GET' });
  ...
  const handlePhotoUpload = async (e) => {
    ...
    try {
      ...
      const resp = await apiRequest(`/api/admin/org/mentor/photo/upload/${user.id}`, { method: 'POST', body: JSON.stringify({ Photo_upload: b64 }) });
      ...
      if (resp?.Photo_upload) {
        ...
        try {
          const pr = await apiRequest(`/api/admin/org/mentor/photo/view/${user.id}`, { method: 'GET' });
  ...
  const handlePhotoDelete = async () => {
    setPhotoUp(true);
    try {
      await apiRequest(`/api/admin/org/mentor/photo/delete/${user.id}`, { method: 'DELETE' });
      setPhoto(null);
  ...
  const saveContact = async () => {
    ...
    try {
      await apiRequest(`/api/mentor/profile/update/${user.id}`, { method: 'PUT', body: JSON.stringify(contactForm) });
  ...
  const saveExpert = async () => {
    ...
    try {
      await apiRequest(`/api/mentor/profile/update/${user.id}`, { method: 'PUT', body: JSON.stringify(expertForm) });
  ...
  const fullName = `${d?.First_Name || ''} ${d?.Last_Name || ''}`.trim() || user?.name || 'Mentor';
  const email = d?.Email || '';
  ...
  return (
    <Box sx={{ maxWidth: 1000, mx: 'auto', px: { xs: 1.5, sm: 2.5, md: 3 }, pt: { xs: 10, sm: 12 }, pb: 6, minHeight: '100vh' }}>
      ...
      <HeroCard fullName={fullName} chipLabel={d?.Mentor_Code || 'Mentor'} profilePhoto={photo}
      ...
      <Box sx={{ display: 'flex', gap: { xs: 2, sm: 3 }, flexDirection: { xs: 'column', md: 'row' }, mb: 3 }}>
        ...
        <Box sx={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: { xs: 2, sm: 3 } }}>
          <Section title="Expertise & Qualifications" subtitle="Skills and experience" delay={.15}
      ...
      <Section title="Account Information" delay={.25}>
        <Box sx={{ display: 'flex', gap: { xs: 1.5, sm: 2 }, flexWrap: 'wrap', mt: 1 }}>
          <AccCard icon={CheckCircleOutlined} label="Status" value={stLabel} iconColor={stColor} chipColor={stColor} delay={.3} />
          <AccCard icon={BusinessOutlined} label="Org ID" value={d?.Org_ID || '\u2014'} iconColor={T.secondary} delay={.35} />
