// src/components/mentor/Batches/QuickTipDialog.jsx
...
const _qtStyles = document.getElementById('qt-dialog-styles') || (() => {
  const s = document.createElement('style');
  s.id = 'qt-dialog-styles';
  s.textContent = `
    @keyframes qtSlideUp {
...
/* ═══════════════════ COMPONENT ═══════════════════ */
const QuickTipDialog = ({ open, onClose, batch }) => {
  ...
  const fetchTips = useCallback(async () => {
    ...
    try {
      ...
      const data = await apiRequest(`/api/mentor/batch/tips/${userId}/${batchId}`, {
        method: 'GET',
      });
  ...
  const handleCreateTip = async () => {
    ...
    try {
      ...
      await apiRequest('/api/mentor/batch/tip/create', {
        method: 'POST',
        body: JSON.stringify({
          mentor_id: userId,
          batch_id: batchId,
  ...
  const handleUpdateTip = async (tipId) => {
    ...
    try {
      const { userId, orgId } = getUserContext();
      await apiRequest(`/api/mentor/batch/tip/update/${tipId}`, {
        method: 'PUT',
        body: JSON.stringify({
          mentor_id: userId,
          org_id: orgId,
  ...
  const handleDeleteTip = async (tipId) => {
    ...
    try {
      const { userId, orgId } = getUserContext();
      await apiRequest(`/api/mentor/batch/tip/delete/${tipId}?mentor_id=${userId}&org_id=${orgId}`, {
        method: 'DELETE',
      });
          ...
          <Box sx={{ px: 3, py: 2, minHeight: 120, maxHeight: 360, overflowY: 'auto' }}>
            {loading ? (
              ...
              <Box sx={{ textAlign: 'center', py: 4 }}>
                ...
                >
                  <FormatQuote sx={{ color: '#0ea5e9', fontSize: 26 }} />
                </Box>
                ...
                {tips.map((tip, index) => (
                  <Box
                    ...
                    sx={{
                      ...
                      border: '1px solid #e8ecf2',
                      transition: 'all 0.3s ease',
                      animation: `qtSlideUp 0.35s ease-out ${index * 0.06}s both`,
                      '&:hover': {
                        background: '#f0f4ff',
                        borderColor: '#bfdbfe',
                            ...
                            <Tooltip title="Edit tip" arrow>
                              <IconButton
                                ...
                                sx={{
                                  ...
                                  '&:hover': { background: 'rgba(14,165,233,0.12)', borderColor: '#0ea5e9', transform: 'translateY(-1px)' },
                                }}
                              >
                                <Edit sx={{ fontSize: 14 }} />
                              </IconButton>
