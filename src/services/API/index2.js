...
const getWebSocketURL = () => {
  try {
    ...
    return `${wsProto}//${base.host}`; // e.g. wss://192.168.48.201:8090
  } catch (e) {
...
export const createInterviewSession = async () => {
  try {
    console.log('🚀 Creating enhanced interview session...');
       ...
       // Get student_id from localStorage (set during login by authService)
    const studentId = localStorage.getItem('student_id');
    if (!studentId) {
      throw new Error('No student_id found. Please log in again.');
    }

    // Primary GET attempt with student_id
    let response;
    try {
      response = await assessmentApiRequest(`/weekly_interview/start_interview?student_id=${studentId}`, {
        method: 'GET',
      });
    } catch (e) {
...
// ===== BIOMETRIC: Pre-interview face verification gate =====
export const verifyFaceGate = async (studentId, imageBase64) => {
  try {
    console.log('🔐 Verifying face before interview...');

    // Get student_id from multiple sources (fallback chain)
    ...
    // Strip data URL prefix if present (canvas.toDataURL includes "data:image/jpeg;base64,")
    const cleanBase64 = imageBase64.includes('base64,') 
      ? imageBase64.split('base64,')[1] 
      : imageBase64;

   console.log('🔐 student_id resolved:', resolvedStudentId);
    console.log('🔐 image_base64 length:', cleanBase64?.length || 0);

    // Send as FormData — backend uses Form() parameters, not JSON body
    const formData = new FormData();
    formData.append('student_id', resolvedStudentId.toString());
    formData.append('image_base64', cleanBase64);

    const response = await assessmentApiRequest(
      `/weekly_interview/verify_face_gate`, {
      method: 'POST',
      body: formData,
    });
    ...
    return {
      ...
      canProceed: response.can_proceed || false,
    };
  } catch (error) {
    ...
    return {
      verified: false,
      similarity: 0,
      error: error.message || 'Face verification request failed',
